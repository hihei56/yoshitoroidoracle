...
const THREAT_REGEX = new RegExp([
    '殺','死ね','しね','死んでしまえ','死にやがれ','さっさと死ね','死ねよ','死んでろ',
    '殺す','ころす','殺してやる','ぶっ殺','刺し殺','撃ち殺','絞め殺',
    '爆破','爆殺','刺す','刺してやる','首を絞め','首を切る(?:ぞ|よ)',
    'kill\\s*you',"i'?ll\\s*kill",'gonna\\s*kill',
    '自殺しろ','自殺して(?:くれ|ください|ね|よ)',
    '死んでください',
    'テロ(?:を起こせ|しろ|実行)',
    '大量虐殺(?:しろ|せよ|万歳)',
    'mass\\s*shoot(?:ing)?',
...
const DRUG_REGEX = new RegExp([
    ...
    '大麻','マリファナ','危険ドラッグ','脱法ドラッグ','合法ドラッグ',
    'ガンジャ','グラス','アヘン','モルヒネ','フェンタニル',
    'シャブ','やく(?:を|買|売|やる)',
    '粉(?:買|売|やる|くれ)','いい粉(?:ある|売)',
...
const SELF_HARM_PROMO_REGEX = new RegExp([
    ...
    '飛び降りる(?:場所|方法)',
    // 死の誘引
    '死に方(?:を教|教えて)','楽に死ぬ(?:方法|には)','安楽死(?:方法|のやり方)',
    '死にたい','死んでしまいたい','消えてしまいたい','もう生きていたくない',
    '死ぬことしか(?:考え|頭に)',
    // OD・薬物
    'od(?:の仕方|やり方|方法|量|して死)',
    '睡眠薬(?:で死|を飲んで死)',
...
const HATE_REGEX = new RegExp([
    // 民族・人種差別語
    'チャンコロ','チョンコ',
    'ニガー','ニガ(?=[^ーな-んア-ン]|$)',  // N-word (ニガタ・ニガい等は除外)
    'n\\W*i\\W*g\\W*g\\W*[ae]r',
    '支那(?:人め|め|野郎|女め)',
    // 黒人への差別
    '黒人(?:は|が|を)?(?:猿|ゴキブリ|害虫|消えろ|出て行け|死ね|帰れ|滅べ|嫌い)',
    '(?:猿|ゴキブリ|害虫)(?:黒人|アフリカ(?:人)?)',
    // ユダヤへの差別・陰謀論
    'ユダヤ(?:人)?(?:は|が|め|野郎)?(?:消えろ|死ね|出て行け|陰謀|支配|世界征服)',
    'ホロコースト(?:は嘘|なんてない|捏造)',
    'アウシュビッツ(?:は嘘|捏造)',
    // 在日・移民差別
    ...
    // 朝鮮・韓国差別
    '朝鮮(?:人)?(?:め|野郎|女)?(?:帰れ|消えろ|死ね|うせろ)',
    '韓国(?:人)?(?:め|野郎|女)?(?:帰れ|消えろ|死ね|うせろ)',
    // 宗教・属性差別
    '(?:ムスリム|イスラム|ユダヤ|LGBT|障害者)(?:は)?(?:消えろ|いなくなれ|死ね|殺せ)',
    // 民族浄化
    '(?:人種|民族|外国人)(?:を)?(?:浄化|根絶|駆逐)',
    'ethnic\\s*cleansing','genocide\\s*(?:now|all)',
    // 生命価値否定
    '生きるに値しない命',
    '生きる価値(?:の)?ない(?:命|人間|存在)',
    '(?:障害者|精神障害者|知的障害者)(?:は)?(?:生きる価値がない|社会のお荷物|不要な存在)',
    // 性差別
    '(?:女|ブス|デブ)(?:は)?(?:黙れ|消えろ|死ね|生きる価値)',
    '(?:女|女性)(?:は)?(?:道具|奴隷|所有物)',
].join('|'), 'i');
